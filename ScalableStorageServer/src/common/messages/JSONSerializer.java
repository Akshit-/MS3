...
public class JSONSerializer {
	...
	public static String marshal(KVMessage kvmsg) {
		...
		return /*new TextMessage(*/objectBuilder.toString()/*)*/;
	}
	...
	public static TextMessage marshalKVAdminMsg(List<MetaData> list,
			Commands command, String destination, String range) {

		JsonObjectBuilder builder = Json.createObjectBuilder()
				.add("adminMsg", true).add("command", command.ordinal())
				.add("range", range).add("destination", destination);

		if (list != null) {
			JsonArrayBuilder array = Json.createArrayBuilder();

			for (MetaData item : list) {
				array.add(Json.createObjectBuilder().add("ip", item.getIP())
	...
	public static KVAdminMessageImpl unmarshalKVAdminMsg(TextMessage txtMsg) {
		...
		for (int i = 0; i < jarray.size(); i++) {
			...
			meta = new MetaData(jarray.getJsonObject(i).getString("ip"), jarray
					.getJsonObject(i).getString("port"), jarray
					.getJsonObject(i).getString("start"), jarray.getJsonObject(
					i).getString("end"));

			metaDatas.add(meta);
		}

		return new KVAdminMessageImpl(
				metaDatas,
				KVAdminMessageImpl.getCommandType(jsonObject.getInt("command")),
				jsonObject.getString("range"), jsonObject
